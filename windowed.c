...
static struct timeval last_drop, current_time;
...
void game_loop(Board game_board, GLFWwindow* window) {
	while(!glfwWindowShouldClose(window)) {
		...
		if ((current_time.tv_sec - last_drop.tv_sec) * 1000000 +
			(current_time.tv_usec - last_drop.tv_usec) > 500000) {
			pthread_mutex_lock(&movement_mutex);
